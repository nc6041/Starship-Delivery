...
//Graph + dijkstra's
class StarshipGraph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        //Deals with if start or end vertex doesn't exist

        if (!vertices.containsKey(start) || !vertices.containsKey(end)) {
            throw new NoSuchElementException("Vertices do not yet exist");
        }

        //Store vertices in variables
        Vertex startVertex = vertices.get(start);
        Vertex endVertex = vertices.get(end);
        //visited set that will keep track of vertices with the shortest paths already found.
        Set<Vertex> visited = new HashSet<>();
        //Priority Queue for logic and to help with algorithm efficiency.
        PriorityQueue<Path> pathQueue = new PriorityQueue<>();

        //Add source to pathQueue to explore edges and visited set as it's the first one visited
        pathQueue.add(new Path(startVertex));
        visited.add(startVertex);

        Path minPath = null;

        //While all vertex not visited
        while (visited.size() != getVertexCount()) {

            //Terminating condition. If there isn't any more edges to consider Path is either found or there isn't one.
            if (pathQueue.isEmpty()) {
                if (!minPath.end.equals(endVertex)) {
                    throw new NoSuchElementException("No path found form start to end vertex");
                }
                return minPath;
            }


            //gets the current minPath from the priority queue
            minPath = pathQueue.remove();

            // If already visited then minimum path is found already for this vertex, this will take care of multiple
            // Edges leading to the same vertex
            if (visited.contains(minPath.end) && !minPath.start.equals(minPath.end)) {
                continue;
            }

            //Process neighboring edges

            //for each edge leaving from minPath consider it for the following
            for (Edge edge : minPath.end.edgesLeaving) {

                //if this edge is not already visited
                if (!visited.contains(edge.target)) {
                    //Add path to queue
                    pathQueue.add(new Path(minPath, edge));
                }

            }
            // Adds end vertex of the min cost path to visited because since all the new edges were added to priority
            // queue and thus considered. So the vertex of at the top of the queue should be the min path of all the
            // new and old paths considered. Thus, this vertex's shortest path is found.
            visited.add(minPath.end);

            //Thus, if we find a min path that already leads to the end then we return that path, because by Dijkstra's
            // Algorithm definition that must be the minimum cost path to this vertex
            if (minPath.end.equals(endVertex)) {
                return minPath;
            }
        ...
        //If we never find a min path that ends at vertex end then we
        throw new NoSuchElementException("No path found form start to end vertex");
    }
